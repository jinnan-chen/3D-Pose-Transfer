...
def smal_batch_inverse_kinematics_transform_naive(
    ...
    # TODO
    if global_orient=='est':
        global_orient_mat = batch_get_pelvis_orient(
            rel_pose_skeleton.clone(), rel_rest_pose.clone(), parents, children, dtype)
    #     import ipdb 
    #     ipdb.set_trace()
    elif global_orient=='one':
        global_orient_mat = torch.eye(3, dtype=dtype, device=device).reshape(1, 3, 3)
    ...
    rot_mat_chain = torch.zeros((batch_size, 33, 3, 3), dtype=torch.float32, device=pose_skeleton.device)
    ...
    if leaf_thetas is not None:
        leaf_cnt = 0
        leaf_rot_mats = leaf_thetas.view([batch_size, 6, 3, 3])
    ...
    for idx_lev in range(1, len(idx_levs)):
        ...
        if idx_lev == len(idx_levs)-1:
            ...
            if phis=='zero':
                ident = torch.eye(3, dtype=dtype, device=device).reshape(1, 1, 3, 3)
                rot_mat = torch.matmul(rot_mat_loc,  ident )
            else:
            # (B, K, 1, 1)
                # import ipdb
                # ipdb.set_trace()
            ...
            #     print(
            #         -1, idx_lev, rot_mat_loc[idx], rot_mat_spin[idx],
            #         (torch.det(rot_mat_loc) > 0).all(), 
            #         (torch.det(rot_mat_spin) > 0).all(),
...
def batch_inverse_kinematics_transform(
        ...
        phis,
        rest_pose,
        children, parents, dtype=torch.float32, train=True,
        leaf_thetas=None):
    """
    Applies a batch of inverse kinematics transform to the joints
    ...
    for i in range(1, parents.shape[0]):
        ...
        if children[i] == -3:
            ...
            # print(i)
            # print('rot_num',len(rot_mat_local))
            # print( rel_rest_pose[:, i])
            # print(rotate_rest_pose[:, parents[i]])
            # print(rot_mat_chain[parents[i]])
            ...
            child_final_loc = final_pose_skeleton[:, children[i]]-final_pose_skeleton[:,i]
            ##naive
            if not train:
    ...
    # (B, K + 1, 3, 3)
    rot_mats = torch.stack(rot_mat_local, dim=1)
    rot_mat_chain= torch.stack(rot_mat_chain, dim=1)
    return rot_mats,  rot_mat_chain,global_orient_mat
