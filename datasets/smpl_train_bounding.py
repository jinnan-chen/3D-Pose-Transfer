...
class smpltrainbounding(torch.utils.data.Dataset):
    MESH_FILE_EXT = 'obj'
    ...
    def modify_commandline_options(parser):
        ...
        parser.add_argument('--smpl_root', type=str, default='../npt-data', help='')
    ...
    def get_item_by_name(self, name, sample_mesh=False, load_mesh=False,mixamo=True):        
        if mixamo:
            ...
            # import ipdb
            # ipdb.set_trace()
            tname=name[:15]+'_710.obj'
            tmesh_path = self._get_mixamo_path(tname)
            try:
                # import ipdb
                # ipdb.set_trace()
                # trimesh.load_mesh(mesh_path).bounds,dtype=torch.float
                bounding1,bounding2=torch.tensor(trimesh.load_mesh(mesh_path).bounds,dtype=torch.float)
                tbounding1,tbounding2=torch.tensor(trimesh.load_mesh(tmesh_path).bounds,dtype=torch.float)
        ...
        V_mesh=V_mesh-(bounding1 + bounding2) / 2
        # tpivet=torch.mm(j_regressor,tV_mesh)[0]
        
        tV_mesh=tV_mesh-(tbounding1 + tbounding2) / 2
        points=V_mesh
        ...
        result = {'shape': shape, 'tshape': tV_mesh, 'file': name}
    ...
    def get_sample(self, index):
        ...
        name='id'+str(pose_mesh_i)+'_'+str(pose_mesh_p)+'.obj'
        name_2='id'+str(identity_mesh_i)+'_'+str(identity_mesh_p)+'.obj'
        name_3='id'+str(pose_mesh_i)+'_'+str(identity_mesh_p)+'.obj'
