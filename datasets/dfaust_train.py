...
class dfausttrain(torch.utils.data.Dataset):
    MESH_FILE_EXT = 'obj'
    ...
    def modify_commandline_options(parser):
        ...
        parser.add_argument('--smpl_root', type=str, default='../registrations', help='')
    ...
    def get_item_by_name(self, name, sample_mesh=False, load_mesh=False,mixamo=True):        
        if mixamo:
            ...
            try:
                ...
                tV_mesh, tF_mesh = pytorch3d.io.load_ply(tmesh_path)
                
                pivet=torch.mm(j_regressor,V_mesh)[0]
                tpivet=torch.mm(j_regressor,tV_mesh)[0]
                
                npt_pivet=torch.tensor([-0.0030,-0.3300,0.0800])
                V_mesh=V_mesh-pivet+npt_pivet
                tV_mesh=tV_mesh-tpivet+npt_pivet
            except Exception:  
                print(name)          
    ...
    def get_sample(self, index):
        ...
        identity_mesh_i=np.random.randint(0,8)
        identity_mesh_p=np.random.randint(0,10)
        # pose_mesh_i = np.random.randint(0,16)
        # pose_mesh_p = np.random.randint(200,600)
        # tr_reg_000.ply
        for i in range(10):
            pose_mesh_i=np.random.randint(0,8)
            pose_mesh_p=np.random.randint(0,10)
            if (identity_mesh_i!=pose_mesh_i) and (identity_mesh_p!=pose_mesh_p):
                break

        name='tr_reg_0'+str(pose_mesh_i)+str(pose_mesh_p)+'.ply'
        name_2='tr_reg_0'+str(identity_mesh_i)+str(identity_mesh_p)+'.ply'
        name_3='tr_reg_0'+str(pose_mesh_i)+str(identity_mesh_p)+'.ply'
