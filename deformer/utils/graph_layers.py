...
class GraphResBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.conv = GraphConvolution(out_channels // 2, out_channels // 2)
    ...
    def forward(self, x,A):
        ...
        y = self.conv(y.transpose(1,2),A).transpose(1,2)
...
#     def __init__(self, pool_mat):
#         super(Pool, self).__init__(flow='target_to_source')
#         self.pool_mat = pool_mat
#
#     def forward(self, x):
#         x = x.transpose(0, 1)
#         out = self.propagate(edge_index=self.pool_mat._indices(), x=x, norm=self.pool_mat._values(),
#                              size=self.pool_mat.size())
